...
class FabricWorkspaceManager(WorkspaceManager):
    ...
    async def reconcile(self, workspace_params: FabricWorkspaceParams) -> None:
        ...
        if workspace_info.display_name != workspace_params.name:
            ...
            self.logger.info(f"Successfully updated display name for workspace '{workspace_params.name}'")
        else:
            self.logger.info(f"Workspace '{workspace_params.name}' display name is already correct")
        ...
        if workspace_info.description != workspace_params.description:
            ...
            self.logger.info(f"Successfully updated description for workspace '{workspace_params.name}'")
        else:
            self.logger.info(f"Workspace '{workspace_params.name}' description is already correct")
        ...
        if as_capacity.default_dataset_storage_mode != workspace_params.dataset_storage_mode:
            ...
            self.logger.info(f"Successfully updated datasetStorageMode for workspace '{workspace_params.name}'")
    ...
    async def assign_workspace_storage_reader(
        ...
        self.logger.info(f"Assigning role '{role}' to workspace identity '{assignee}' for storage scope: {scope}")

        try:
            self.az_cli.run(["role", "assignment", "create", "--assignee", assignee, "--role", role, "--scope", scope])
